    ...
    mNeedSolidColorLayer(false)
{
    ...
    if (mMaxSrcLayerNum > 1) mNeedSolidColorLayer = true;
    ...
    if (freeDstBuf.bufferHandle != NULL)
        ...
        if (mAssignedDisplay != NULL) {
            ...
            freeDstBuf.acrylicReleaseFenceFd = fence_close(freeDstBuf.acrylicReleaseFenceFd,
                    mAssignedDisplay, FENCE_TYPE_DST_RELEASE, FENCE_IP_G2D);
        }
    }
 ...
 * @return int32_t
 */
int32_t ExynosMPP::setOutBuf(buffer_handle_t outbuf, int32_t fence, ExynosDisplay *display) {
...
int32_t ExynosMPP::setupLayer(exynos_mpp_img_info *srcImgInfo, struct exynos_image &src, struct exynos_image &dst)
{
    ...
    uint32_t bufferNum = getBufferNumOfFormat(src.format);
    android_dataspace_t dataspace = src.dataSpace;

    if (bufferNum == 0)
    {
        MPP_LOGE("%s:: Fail to get bufferNum(%d), format(0x%8x)", __func__, bufferNum, src.format);
        return -EINVAL;
    }
    ...
    if (!(src.layerFlags & EXYNOS_HWC_DIM_LAYER)) {
        ...
        if (getBufLength(srcHandle, MAX_HW2D_PLANES, bufLength, src.format, src.fullWidth, src.fullHeight) != NO_ERROR) {
            MPP_LOGE("%s:: invalid bufferLength(%zu, %zu, %zu), format(0x%8x)", __func__,
                    bufLength[0], bufLength[1], bufLength[2], src.format);
            return -EINVAL;
        }
    ...
    if (src.format == HAL_PIXEL_FORMAT_EXYNOS_YCbCr_420_SP_M_PRIV) {
        srcImgInfo->mppLayer->setImageType(HAL_PIXEL_FORMAT_EXYNOS_YCbCr_420_SP_M, dataspace);
    } else {
        srcImgInfo->mppLayer->setImageType(src.format, dataspace);
    }
...
int32_t ExynosMPP::setupDst(exynos_mpp_img_info *dstImgInfo)
{
    ...
    if (mPhysicalType == MPP_G2D) {
        setFenceName(dstImgInfo->acrylicReleaseFenceFd, FENCE_G2D_DST_DPP);
        /* Might be closed next frame */
        setFenceInfo(dstImgInfo->acrylicReleaseFenceFd, mAssignedDisplay,
                FENCE_TYPE_DST_RELEASE, FENCE_IP_G2D, FENCE_TO);
    } else if (mPhysicalType == MPP_MSC) {
        setFenceName(dstImgInfo->acrylicReleaseFenceFd, FENCE_MSC_DST_DPP);
        /* Might be closed next frame */
        setFenceInfo(dstImgInfo->acrylicReleaseFenceFd, mAssignedDisplay,
                FENCE_TYPE_DST_RELEASE, FENCE_IP_MSC, FENCE_TO);
    } else {
        MPP_LOGE("%s:: invalid mPhysicalType(%d)", __func__, mPhysicalType);
    }
    ...
    if ((mLogicalType == MPP_LOGICAL_G2D_COMBO) &&
        ...
        dstImgInfo->acrylicReleaseFenceFd = fence_close(dstImgInfo->acrylicReleaseFenceFd,
                mAssignedDisplay, FENCE_TYPE_DST_RELEASE, FENCE_IP_G2D);
        ...
        dstImgInfo->acrylicReleaseFenceFd =
            hwcCheckFenceDebug(mAssignedDisplay, FENCE_TYPE_DST_RELEASE, FENCE_IP_G2D, dstImgInfo->acrylicReleaseFenceFd);
        mAcrylicHandle->setCanvasBuffer(bufFds, bufLength, bufferNum,
                dstImgInfo->acrylicReleaseFenceFd, attribute);
    }
    ...
    MPP_LOGD(eDebugMPP|eDebugFence, "\tImageBuffer handle: %p, fds[%d, %d, %d], bufLength[%zu, %zu, %zu], bufferNum: %d, releaseFence: %d, attribute: %d",
...
int32_t ExynosMPP::doPostProcessingInternal()
{
    ...
    MPP_LOGD(eDebugFence, "setupDst ++ mDstImgs[%d] acrylicReleaseFenceFd(%d)",
            mCurrentDstBuf, mDstImgs[mCurrentDstBuf].acrylicReleaseFenceFd);

    setupDst(&mDstImgs[mCurrentDstBuf]);

    MPP_LOGD(eDebugFence, "setupDst -- mDstImgs[%d] acrylicReleaseFenceFd(%d) closed",
            mCurrentDstBuf, mDstImgs[mCurrentDstBuf].acrylicReleaseFenceFd);
    ...
    if (acrylicReturn == false) {
        ...
        for(size_t i = 0; i < sourceNum; i++) {
            ...
            private_handle_t *srcHandle = NULL;
            if (mAssignedSources[i]->mSrcImg.bufferHandle != NULL)
                srcHandle = private_handle_t::dynamicCast(mAssignedSources[i]->mSrcImg.bufferHandle);
            if (getBufLength(srcHandle, MAX_HW2D_PLANES, bufLength, mAssignedSources[i]->mSrcImg.format,
                        mAssignedSources[i]->mSrcImg.fullWidth, mAssignedSources[i]->mSrcImg.fullHeight) != NO_ERROR) {
                MPP_LOGE("src[%zu]: bufferLength(%zu, %zu, %zu), format(0x%8x)", i,
                        bufLength[0], bufLength[1], bufLength[2], mAssignedSources[i]->mSrcImg.format);
            }

        }
        if (mAssignedDisplay == NULL) {
            MPP_LOGE("assigned display is NULL");
        } else {
            MPP_LOGE("dst mXres: %d, mYres: %d",
                    mAssignedDisplay->mXres, mAssignedDisplay->mYres);
            private_handle_t *dstHandle = mDstImgs[mCurrentDstBuf].bufferHandle;
            if (getBufLength(dstHandle, MAX_HW2D_PLANES, bufLength, mDstImgs[mCurrentDstBuf].format,
                        dstHandle->stride, dstHandle->vstride) == NO_ERROR) {
                MPP_LOGE("dst[%d] bufferLength(%zu, %zu, %zu), format(0x%8x)",
                        mCurrentDstBuf,
        ...
        if ((mAssignedDisplay != NULL) &&
            ...
            if (((ExynosVirtualDisplay *)mAssignedDisplay)->mIsWFDState == (int)LLWFD) {
                if (usingFenceCnt != 0) // Use no fences
                    outFences[dstBufIdx] = fence_close(outFences[dstBufIdx],
            ...
            if (mUseM2MSrcFence) {
                if (((ExynosVirtualDisplay *)mAssignedDisplay)->mIsWFDState != (int)GOOGLEWFD) {
                    for (size_t i = 0; i < sourceNum; i++)
                        outFences[i] = fence_close(outFences[i],
        ...
        if (usingFenceCnt == 0) { // Use no fences
            ...
            for(size_t i = 0; i < sourceNum; i++) {
                if (mUseM2MSrcFence)
                    mSrcImgs[i].acrylicReleaseFenceFd =
                        hwcCheckFenceDebug(mAssignedDisplay, FENCE_TYPE_SRC_RELEASE, FENCE_IP_G2D, outFences[i]);
                else
                    mSrcImgs[i].acrylicReleaseFenceFd = -1;
            ...
            if (mPhysicalType == MPP_G2D) {
                if (mDstImgs[mCurrentDstBuf].acrylicAcquireFenceFd >= 0)
                    mDstImgs[mCurrentDstBuf].acrylicAcquireFenceFd =
                        fence_close(mDstImgs[mCurrentDstBuf].acrylicAcquireFenceFd,
                                mAssignedDisplay, FENCE_TYPE_DST_ACQUIRE, FENCE_IP_G2D);
                mDstImgs[mCurrentDstBuf].acrylicAcquireFenceFd =
                    hwcCheckFenceDebug(mAssignedDisplay, FENCE_TYPE_DST_ACQUIRE, FENCE_IP_G2D, outFences[dstBufIdx]);
            } else if (mPhysicalType == MPP_MSC) {
                if (mDstImgs[mCurrentDstBuf].acrylicAcquireFenceFd >= 0)
                    mDstImgs[mCurrentDstBuf].acrylicAcquireFenceFd =
                        fence_close(mDstImgs[mCurrentDstBuf].acrylicAcquireFenceFd,
                                mAssignedDisplay, FENCE_TYPE_DST_ACQUIRE, FENCE_IP_MSC);
                mDstImgs[mCurrentDstBuf].acrylicAcquireFenceFd =
                    hwcCheckFenceDebug(mAssignedDisplay, FENCE_TYPE_DST_ACQUIRE, FENCE_IP_MSC, outFences[dstBufIdx]);
            }
            MPP_LOGD(eDebugFence, "mDstImgs[%d] acrylicAcquireFenceFd: %d , outFences[%d]",
                    mCurrentDstBuf, mDstImgs[mCurrentDstBuf].acrylicAcquireFenceFd, dstBufIdx);
        }
 ...
 * by ExynosDisplay
 */
int32_t ExynosMPP::setDstReleaseFence(int releaseFence, ExynosDisplay *display)
{
    ...
    if (dstBufIndex < 0 || dstBufIndex >= NUM_MPP_DST_BUFS(mLogicalType)) {
        // TODO fence_close..
        releaseFence = fence_close(releaseFence, mAssignedDisplay, FENCE_TYPE_DST_RELEASE, FENCE_IP_ALL);
        mPrivDstBuf = mCurrentDstBuf;
        return -EINVAL;
    }

    if (mDstImgs[dstBufIndex].acrylicReleaseFenceFd >= 0) {
        MPP_LOGD(eDebugFence,"mDstImgs[%d].acrylicReleaseFenceFd: %d is closed", dstBufIndex,
                mDstImgs[dstBufIndex].acrylicReleaseFenceFd);
        fence_close(mDstImgs[dstBufIndex].acrylicReleaseFenceFd, mAssignedDisplay,
                FENCE_TYPE_DST_ACQUIRE, FENCE_IP_ALL);
    }
    if (mPhysicalType == MPP_MSC)
        mDstImgs[dstBufIndex].acrylicReleaseFenceFd =
            hwcCheckFenceDebug(mAssignedDisplay, FENCE_TYPE_DST_RELEASE, FENCE_IP_MSC, releaseFence);
    else
        mDstImgs[dstBufIndex].acrylicReleaseFenceFd =
            hwcCheckFenceDebug(mAssignedDisplay, FENCE_TYPE_DST_RELEASE, FENCE_IP_G2D, releaseFence);

    MPP_LOGD(eDebugFence,"->mDstImgs[%d].acrylicReleaseFenceFd: %d", dstBufIndex,
            mDstImgs[dstBufIndex].acrylicReleaseFenceFd);
...
int64_t ExynosMPP::isSupported(ExynosDisplay &display, struct exynos_image &src, struct exynos_image &dst)
{
    ...
    if (rot_dst.w > maxDstWidth)
        ...
        return -eMPPExeedMinDstWidth;
    else if (!!(src.layerFlags & EXYNOS_HWC_DIM_LAYER)) // Dim layer
    {
...
{
    mDstAllocatedSize = width * height;
}

uint32_t ExynosMPP::getDstAllocSize()
{
...
bool ExynosMPP::hasEnoughCapa(ExynosDisplay *display, struct exynos_image &src, struct exynos_image &dst)
{
    ...
    if ((mReservedDisplay == display->mDisplayId) && (mPreAssignedCapacity > (float)0.0))
        ...
        uint32_t &formatIndex, uint32_t &rotIndex, uint32_t &scaleIndex, struct exynos_image &criteria)
{
    ...
    if (isFormat10BitYUV420(criteria.format))
        ...
        struct exynos_image &dst)
{
    ...
    if (mPhysicalType == MPP_G2D) {
        ...
        if (mLogicalType == MPP_LOGICAL_MSC_COMBO &&
                ...
                (display->is2StepBlendingRequired(mAssignedSources[0]->mSrcImg, mDstImgs[mCurrentDstBuf].bufferHandle)))
            capacity += max(srcCapacity, dstCapacity);
...
bool ExynosMPP::addCapacity(ExynosMPPSource* mppSource)
{
    ...
    if (mPhysicalType == MPP_G2D) {
        ...
        mUsedCapacity = getRequiredCapacity(mAssignedDisplay, mppSource->mSrcImg, mppSource->mMidImg);
    }
